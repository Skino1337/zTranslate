namespace GOTHIC_NAMESPACE
{
    ...
    {
        // auto menu_name = std::string((const char*)menu->GetName());
        ...
        //         continue;
        //
        //     auto menu_item_name = std::string(menu_item->GetName().ToChar());
        //     log << "menu_item [" << i << "] name: " << menu_item_name <<std::endl;
        //
        //
    ...
    zCMenuItem* get_menu_item_in_menu_by_name(zCMenu* menu, const std::string & menu_item_name)
    {
        for (int i = 0; i < menu->m_listItems.GetNum(); ++i)
        {
    ...
    zCMenuItem* get_menu_item_in_menu_by_name_substring(zCMenu* menu, const std::string & menu_item_name_substring)
    {
        for (int i = 0; i < menu->m_listItems.GetNum(); ++i)
        {
            std::string iterate_menu_item_name = (const char*)menu->m_listItems[i]->GetName();
            if (string_have_substring_case_insensitive(iterate_menu_item_name, menu_item_name_substring))
                return menu->m_listItems[i];
        }
    ...
    void move_menu_items_in_menu()
    {
        if (main_menu_item_exit == nullptr)
            return;

        int x = 0, y = 0;
        main_menu_item_exit->GetPos(x, y);

        if (main_menu_exit_y < 0)
            main_menu_exit_y = y + menu_y_offset;

        if (y < main_menu_exit_y)
            main_menu_item_exit->SetPos(x, main_menu_exit_y);
    }
    ...
    void create_and_insert_menu_item_language_to_menu(zCMenu* menu)
    {
        ...
        std::vector<int> menu_items_id_valid_vec;
        std::vector<zCMenuItem*> menu_items_valid_vec;
        for (int i = 0; i < menu->m_listItems.GetNum(); ++i)
        {
            if (menu->m_listItems[i]->m_parItemFlags & IT_SELECTABLE)
            {
                menu_items_valid_vec.push_back(menu->m_listItems[i]);
                menu_items_id_valid_vec.push_back(i);
            }
        }

        if (menu_items_valid_vec.size() < 3)
            return;

        main_menu_item_exit = menu_items_valid_vec[menu_items_valid_vec.size() - 1];
        if (main_menu_item_exit == nullptr)
            return;

        auto menu_item_credits = menu_items_valid_vec[menu_items_valid_vec.size() - 2];
        if (menu_item_credits == nullptr)
            return;

        auto menu_item_intro = menu_items_valid_vec[menu_items_valid_vec.size() - 3];
        ...
        main_menu_item_exit->GetPos(x, menu_item_exit_y);
        ...
        move_menu_items_in_menu();

        auto menu_item_language = create_menu_item_from_other(main_menu_item_exit);
        ...
        for (int i = 0; i < menu->m_listItems.GetNum(); ++i)
        {
            if (i == menu_items_id_valid_vec.back()) // last element (exit)
                list_menu_items.InsertEnd(menu_item_language);

            list_menu_items.InsertEnd(menu->m_listItems[i]);
        }

        menu->m_listItems = list_menu_items;
    }

    void create_menu_language()
    {
        // auto menu_audio = zCMenu::GetByName("MENU_OPT_AUDIO");
        // if (menu_audio)
        // {
        //     menu_log(menu_audio);
        // }
    ...
    void menu_loop()
    {
        ...
        if (main_menu_name.empty())
        {
            ...
            for (int i = 0; i < zCMenu::menuList.GetNum(); ++i)
            {
                menu_log(zCMenu::menuList[i]);
            }
        }
